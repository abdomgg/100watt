...
class PurchaseOrder(models.Model):
    ...
    approver_ids = fields.One2many('purchase.order.approver.line', 'order_id', string='Approvers')
    ...
    def _compute_approval_required(self):
        for order in self:
            if order.state == 'draft' and order.user_id:
                order.approval_required = bool(order.user_id.purchase_approver_ids)
            else:
    ...
    def create(self, vals):
        try:
            ...
            if 'create' in error_msg.lower() and 'access' in error_msg.lower():
                _logger.error('Access rights error creating purchase order: %s', error_msg)
        ...
        has_approvers = bool(user_id.purchase_approver_ids) if user_id else False
    ...
    def _auto_request_approval(self):
        ...
        if not self.user_id or not self.user_id.purchase_approver_ids:
            return
        ...
        for approver in self.user_id.purchase_approver_ids:
            approver_lines |= self.env['purchase.order.approver.line'].create({
                'order_id': self.id,
